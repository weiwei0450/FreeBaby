...
def trim_messages_history(history, context_win, force=False):
    ...
    # [A] 主动预警：context_win * 1.2 时压缩旧消息
    if cost > context_win * 1.2 and not force:
        compress_history_tags(history, keep_recent=6, force=False)
        cost = sum(len(json.dumps(m, ensure_ascii=False)) for m in history)
        print(f'[Debug] After compress: {cost} chars')
    # [B] 强制裁剪阈值：从 3x 降到 1.5x，硬目标 1.0x
    hard_max = min(context_win * 1.5, 40000)  # 不超过40K硬上限
    if cost > hard_max or force:
        compress_history_tags(history, keep_recent=4, force=True)
        target = context_win * 1.0
        if cost > 50000: target = context_win * 0.8  # 极端情况砍更狠
        while len(history) > 6 and cost > target:
            history.pop(0)
...
def _openai_stream(sess, messages):
    ...
    if getattr(sess, 'extra_headers', None): headers.update(sess.extra_headers)
    if api_mode == "responses":
...
class BaseSession:
    ...
    def ask(self, prompt, stream=False):
        def _ask_gen():
            ...
            # [B] 超时保护：120秒超时→中断→裁剪重试
            TIMEOUT_SEC = 120
            def _do_ask(msgs, retry=False):
                nonlocal content, content_blocks
                ...
                def _worker():
                    try:
                        ...
                        chunks = []
                        try:
                            while True: chunks.append(next(gen))
                        except StopIteration as e:
                            content_blocks = e.value or []
                        result_q.put(('ok', chunks))
                    except Exception as e:
                        result_q.put(('err', e))
                    finally:
                        done_event.set()
                ...
                if not done_event.wait(TIMEOUT_SEC):
                    err_event.set()
                    print(f'[Timeout] API {TIMEOUT_SEC}s timeout, trigerring retry...')
                ...
                for chunk in data:
                    ...
                    yield chunk
                return True, None
            ok, err = yield from _do_ask(messages)
            if not ok:
                print(f'[B] ask failed ({err}), trimming aggressively and retrying...')
                with self.lock:
                    trim_messages_history(self.history, self.context_win, force=True)
                    messages2 = self.make_messages(self.history)
                ok2, err2 = yield from _do_ask(messages2, retry=True)
                if not ok2:
                    err_msg = f"!!!Error: [Timeout/Error after retry] {err2}"
                    yield err_msg
                    if not content.startswith("!!!Error:"): content = err_msg
                    content_blocks = [{"type": "text", "text": err_msg}]
...
"""
    ...
    def _build_protocol_prompt(self, messages, tools):
        ...
        user += "=== ASSISTANT ===\n" 
        # [A-final] 最终截断保护：拼接后总长超过40K时截断旧history
        full = system + user
        if len(full) > 40000:
            print(f'[A-final] Prompt too long ({len(full)} chars), truncating history...')
            # 从history_msgs头部开始砍，保留最后40%的user内容
            lines = user.split("=== USER ===\n")
            if len(lines) > 2:
                keep = int(len(lines) * 0.6)
                user = "=== USER ===\n".join(lines[-keep:])
                full = system + user
                print(f'[A-final] After trunc: {len(full)} chars, kept {keep}/{len(lines)} history blocks')
        return full
    ...
    def _parse_mixed_response(self, text):
        ...
        elif ('"name":' in remaining_text or '"name" :' in remaining_text) and ('"arguments":' in remaining_text or '"parameters":' in remaining_text or '"args":' in remaining_text):
            json_match = re.search(r'\{[^{}]*"name"\s*:\s*"[^"]+"\s*,\s*"(?:arguments|parameters|args)"\s*:\s*\{[^}]*\}\s*\}', remaining_text, re.DOTALL)
            if not json_match: json_match = re.search(r'\{.*"name"\s*:.*\}', remaining_text, re.DOTALL)
            if json_match:
