...
sys.path.append(os.path.abspath(os.path.join(script_dir, '..', 'memory')))

import streamlit as st
import time, json, re, threading, queue
from vision_api import ask_vision as vision_analyze
...
def render_sidebar():
    ...
    # ── 📷 图片识图（GPT-4.1-mini）──
    def handle_upload():
        if st.session_state.vision_upload:
            uploaded = st.session_state.vision_upload
            import tempfile, os
            tmp_path = os.path.join(tempfile.gettempdir(), f"st_vis_{uploaded.name}")
            with open(tmp_path, 'wb') as f: f.write(uploaded.read())
            st.session_state.vision_task = tmp_path
            st.session_state.vision_name = uploaded.name

    st.file_uploader("📷 上传/粘贴图片", type=["png","jpg","jpeg","webp","bmp"],
                                label_visibility="collapsed", key="vision_upload", on_change=handle_upload)
    st.caption("支持 Ctrl+V 粘贴、拖拽或点击上传 → GPT-4.1-mini 识图")

    if 'vision_task' in st.session_state:
        # 在此处执行任务，确保只触发一次
        tmp_path = st.session_state.pop('vision_task')
        filename = st.session_state.pop('vision_name')
        
        # ... [此处放置原有的线程+OCR逻辑] ...
        
        # 注入后清除组件状态
        st.session_state.vision_upload = None
        st.rerun()
    # 粘贴监听（Ctrl+V 自动转发到 file_uploader）
    st.markdown("""
    <script>
    (function(){document.addEventListener('paste',function(e){
        for(let item of e.clipboardData.items){
            if(item.type.indexOf('image/')===0){
                e.preventDefault();
                let blob=item.getAsFile();
                let file=new File([blob],'clipboard_'+Date.now()+'.png',{type:'image/png'});
                let dt=new DataTransfer();dt.items.add(file);
                let inputs=parent.document.querySelectorAll('input[type="file"]');
                for(let fi of inputs){if(fi.accept.indexOf('image/')===0){fi.files=dt.files;
                fi.dispatchEvent(new Event('change',{bubbles:true}));break;}}
    }}})})();
    </script>
