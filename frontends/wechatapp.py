...
try:
    from voice_reply import text_to_silk_file
    _VOICE_OK = True
except ImportError:
    _VOICE_OK = False
...
ITEM_IMAGE, ITEM_FILE, ITEM_VIDEO, ITEM_VOICE = 2, 4, 5, 6
...
class WxBotClient:
    ...
    def send_voice(self, to_user_id, file_path, context_token=''):
        return self._send_media(to_user_id, file_path, 5, ITEM_VOICE, 'voice_item', context_token)
...
def on_message(bot, msg):
    ...
    def _handle():
        ...
        if _VOICE_OK:
            try:
                voice_text = _clean(result)[:500]  # 限500字防超时
                if voice_text and voice_text != '...':
                    silk_path = os.path.join(_TEMP_DIR, f'voice_{uuid.uuid4().hex[:8]}.silk')
                    text_to_silk_file(voice_text, silk_path)
                    bot.send_voice(uid, silk_path, context_token=ctx)
                    print(f'[WX] voice sent ok: {silk_path}', file=sys.__stdout__)
                    try: os.remove(silk_path)
                    except: pass
            except Exception as e:
                print(f'[WX] voice err: {e}', file=sys.__stdout__)
...
if __name__ == '__main__':
    # Lock disabled to allow restart
    # try: _lock = socket.socket(socket.AF_INET, socket.SOCK_STREAM); _lock.bind(('127.0.0.1', 19532))
